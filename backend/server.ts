...
const app = express()
app.use(cors({ origin: true, allowedHeaders: ["Content-Type", "x-admin"] }))
...
async function initDb() {
  ...
  // create tables (email UNIQUE to avoid duplicate accounts)
  await db.exec(`
  ...
  // ensure admin user exists (won't duplicate thanks to UNIQUE constraint)
  try {
    await db.run(
  ...
  // sample products to populate the store on init
  ...
  try {
    ...
    if (!row || row.cnt === 0) {
      const insert = await db.prepare("INSERT INTO products (name, price, img, tag) VALUES (?, ?, ?, ?)")
      try {
    ...
    // ignore product insertion errors
  }
}
...
// simple admin check: expects header x-admin: "true"
...
app.post("/users/login", async (req, res) => {
  ...
  const db = await openDb()
  // return is_admin flag as well
  const user = await db.get("SELECT id, name, email, is_admin FROM users WHERE email = ? AND password = ?", [email, password])
  if (user) {
    res.json(user)
  } else {
...
app.post("/users/register", async (req, res) => {
  ...
  res.json({ id, name, email, is_admin: 0 })
})
...
// Product API - public read, admin-protected write
...
app.post("/products", async (req, res) => {
  ...
  const db = await openDb()
  const result = await db.run("INSERT INTO products (name, price, img, tag) VALUES (?, ?, ?, ?)", [name, price, img || "", tag || ""])
...
app.put("/products/:id", async (req, res) => {
  ...
  const { name, price, img, tag } = req.body
  const db = await openDb()
  await db.run("UPDATE products SET name = ?, price = ?, img = ?, tag = ? WHERE id = ?", [name, price, img || "", tag || "", id])
  res.send("Updated")
})
...
// New: record an order (frontend checkout will POST here)
